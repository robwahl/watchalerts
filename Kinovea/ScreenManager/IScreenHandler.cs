...
namespace Kinovea.ScreenManager
{
    ...
    {
        void Screen_SetActiveScreen(AbstractScreen activeScreen);

        void Screen_CloseAsked(AbstractScreen senderScreen);

        void Screen_UpdateStatusBarAsked(AbstractScreen senderScreen);

        void Player_SpeedChanged(PlayerScreen screen, bool bInitialisation);

        void Player_PauseAsked(PlayerScreen screen);

        void Player_SelectionChanged(PlayerScreen screen, bool bInitialization);

        void Player_ImageChanged(PlayerScreen _screen, Bitmap image);

        void Player_SendImage(PlayerScreen screen, Bitmap image);

        void Player_Reset(PlayerScreen screen);

        void Capture_FileSaved(CaptureScreen screen);

        void Capture_LoadVideo(CaptureScreen screen, string filepath);
    }
}