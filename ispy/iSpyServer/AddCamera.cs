...
namespace iSpyServer
{
    public partial class AddCamera : Form
    {
        ...
        private void AddCamera_Load(object sender, EventArgs e)
        {
            ...
            ddlTimestampLocation.Items.AddRange(items: LocRM.GetString("TimeStampLocations").Split(','));
            ...
            if (CameraControl.Camobject.id > -1)
                if (Text != null)
                    Text += string.Format("{0}{1}{2}{3}{4}", Resources.AddCamera_AddCamera_Load___ID__, CameraControl.Camobject.id, Resources.AddCamera_AddCamera_Load___DIR__, CameraControl.Camobject.directory, Resources.AddCamera_AddCamera_Load__);

            if (CameraControl.Camera != null)
            {
                CameraControl.Camera.NewFrame -= Camera_NewFrame;
                CameraControl.Camera.NewFrame += Camera_NewFrame;
            }
