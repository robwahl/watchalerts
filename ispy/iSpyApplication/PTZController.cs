...
namespace iSpyApplication
{
    public class PTZController
    {
        ...
        public void ConfigurePelco()
        {
            ...
            try
            {
                _serialPort = new SerialPort(cfg[0], Convert.ToInt32(cfg[1]), p, Convert.ToInt32(cfg[2]), sb) { WriteTimeout = 2000, ReadTimeout = 2000 };
            }
            catch (Exception ex)
            {
        ...
        private void ProcessIAM(Enums.PtzCommand command)
        {
            if (_cameraControl.Camera != null)
            {
        ...
        private void IAMMove(VideoCaptureDevice d, CameraControlProperty p, int i)
        {
        ...
        private void ProcessOnvif(Enums.PtzCommand command)
        {
            if (PTZProfile != null)
            {
        ...
        private void ProcessOnvifCommand(string name)
        {
            if (PTZProfile != null)
            {
                try
        ...
        private void ProcessPelco(Enums.PtzCommand command, bool usePelcoP)
        {
            //PELCO
            if (_serialPort == null)
            {
        ...
        private void ProcessPelcoCommand(string command, bool usePelcoP)
        {
            //PELCO
            if (_serialPort == null)
            {
        ...
        private void SendPelco(byte[] arr)
        {
